...
#define PROGRAM ((struct bbb_program_harm1*)program)
#define PRINTER ((struct bbb_printer_harm1*)printer)
#define PPROG ((struct bbb_program_harm1*)(printer->program))

/* Cleanup.
 */
 
static void _harm1_program_del(struct bbb_program *program) {
  bbb_wave_del(PROGRAM->wave);
}

static void _harm1_printer_del(struct bbb_printer *printer) {
...
static int _harm1_program_init(struct bbb_program *program,struct bb_decoder *src) {
  ...
  if (bb_decode_raw(&coefv,src,coefc)<0) return -1;
  
  const struct bbb_wave *sine=bbb_wave_get_sine(program->context);
  if (!sine) return -1;
  if (!(PROGRAM->wave=bbb_wave_new())) return -1;
  bbb_wave_generate_harmonics_u8(PROGRAM->wave->v,sine->v,BBB_WAVE_SIZE,coefv,coefc,flags&0x80);
...
static uint32_t _harm1_program_pack_sndid(struct bbb_program *program,uint8_t pid,uint8_t noteid,uint8_t velocity) {
...
static int _harm1_printer_init(struct bbb_printer *printer,uint8_t noteid,uint8_t velocity) {
...
static int _harm1_printer_update(int16_t *v,int c,struct bbb_printer *printer) {
...
const struct bbb_program_type bbb_program_type_harm1={
  .ptid=BBB_PROGRAM_TYPE_harm1,
  .name="harm1",
  .program_objlen=sizeof(struct bbb_program_harm1),
  .printer_objlen=sizeof(struct bbb_printer_harm1),
  .program_del=_harm1_program_del,
  .program_init=_harm1_program_init,
  .program_pack_sndid=_harm1_program_pack_sndid,
  .printer_del=_harm1_printer_del,
  .printer_init=_harm1_printer_init,
  .printer_update=_harm1_printer_update,
};
