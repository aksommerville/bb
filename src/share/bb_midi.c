...
static int bb_midi_file_decode(struct bb_midi_file *file,const void *src,int srcc) {
  ...
  while (bb_decoder_remaining(&decoder)) {
    int chunkid,c;
    const void *v;
    if (bb_decode_intbe(&chunkid,&decoder,4)<0) return -1;
    if ((c=bb_decode_intbelen(&v,&decoder,4))<0) return -1;
    switch (chunkid) {
...
static void bb_midi_track_read_delay(struct bb_midi_file_reader *reader,struct bb_midi_track_reader *track) {
  ...
  int err=bb_vlq_decode(&track->delay,track->track.v+track->p,track->track.c-track->p);
  ...
  switch (status&0xf0) {
    ...
    default: track->status=0; event->chid=BB_MIDI_CHID_ALL; switch (status) {
        ...
        case 0xf0: case 0xf7: {
            event->opcode=BB_MIDI_OPCODE_SYSEX;
            int len,err;
            if ((err=bb_vlq_decode(&len,track->track.v+track->p,track->track.c-track->p))<1) return -1;
            track->p+=err;
