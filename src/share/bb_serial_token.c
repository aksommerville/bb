...
int bb_number_measure(const char *src,int srcc,int *flags) {
  if (!src) return 0;
  ...
  *flags=0;
  ...
  while ((srcp<srcc)&&(bb_digit_eval(src[srcp])>=0)) srcp++;
  ...
  if (bb_digit_eval(src[srcp])>=0) return -1;
  return srcp;
}
...
int bb_string_measure(const char *src,int srcc,int *flags) {
  if (!src) return 0;
  if (srcc<0) { srcc=0; while (src[srcc]) srcc++; }
  if (srcc<2) return 0;
...
int bb_ident_measure(const char *src,int srcc) {
  if (!src) return 0;
  if (srcc<0) { srcc=0; while (src[srcc]) srcc++; }
  if (srcc<1) return 0;
  ...
  int srcp=0;
  while ((srcp<srcc)&&bb_isident(src[srcp])) srcp++;
  return srcp;
}
...
static int bb_json_measure_string(const char *src,int srcc) {
  int srcp=1;
  while (1) {
    if (srcp>=srcc) return -1;
    if (src[srcp]=='\\') {
...
static int bb_json_measure_structure(const char *src,int srcc,char term) {
  int srcp=1;
  while (1) {
    if (srcp>=srcc) return -1;
    int err=-1;
    switch (src[srcp]) {
      case '{': err=bb_json_measure_structure(src+srcp,srcc-srcp,'}'); break;
      case '[': err=bb_json_measure_structure(src+srcp,srcc-srcp,']'); break;
      case '"': err=bb_json_measure_string(src+srcp,srcc-srcp); break;
      default: {
...
static int bb_json_measure_ident_or_number(const char *src,int srcc) {
  int srcp=0;
  while (srcp<srcc) {
...
int bb_json_measure(const char *src,int srcc) {
  if (!src) return 0;
  if (srcc<0) { srcc=0; while (src[srcc]) srcc++; }
  ...
  switch (src[srcp]) {
    ...
    case '[': err=bb_json_measure_structure(src+srcp,srcc-srcp,']'); break;
    case '"': err=bb_json_measure_string(src+srcp,srcc-srcp); break;
    default: err=bb_json_measure_ident_or_number(src+srcp,srcc-srcp); break;
  }
  if (err<1) return -1;
  srcp+=err;
...
int bb_int_eval(int *dst,const char *src,int srcc) {
  if (!src) return -1;
  if (srcc<0) { srcc=0; while (src[srcc]) srcc++; }
  ...
  *dst=0;
  while (srcp<srcc) {
    int digit=bb_digit_eval(src[srcp]);
...
int bb_decsint_repr(char *dst,int dsta,int src) {
  int dstc;
  if (src<0) {
    int limit=-10,i;
    dstc=2;
...
int bb_decuint_repr(char *dst,int dsta,int src,int digitc) {
  int dstc;
  if (digitc<0) {
    dstc=-digitc;
  } else {
...
int bb_float_eval(double *dst,const char *src,int srcc) {
  if (!src) return -1;
  if (srcc<0) { srcc=0; while (src[srcc]) srcc++; }
...
int bb_float_repr(char *dst,int dsta,double src) {
  ...
  dstc+=bb_decuint_repr(dst+dstc,dsta-dstc,(int)whole,0);
  ...
  if (exp) {
    ...
    dstc++;
    dstc+=bb_decuint_repr(dst+dstc,dsta-dstc,exp,0);
  }
...
int bb_string_eval(char *dst,int dsta,const char *src,int srcc) {
    ...
    if (src[srcp]=='\\') {
      ...
      switch (src[srcp++]) {
        ...
        case 'x': {
            ...
            int hi=bb_digit_eval(src[srcp++]);
            int lo=bb_digit_eval(src[srcp++]);
        ...
        case 'u': {
            ...
            if ((ch>=0xd800)&&(ch<0xdc00)&&(srcp<=srcc-6)&&(src[srcp]=='\\')&&(src[srcp+1]=='u')) {
              int lo=0;
              for (i=0;i<4;i++) {
                int digit=bb_digit_eval(src[srcp+2+i]);
            ...
            int err=bb_utf8_encode(dst+dstc,dsta-dstc,ch);
...
static void bb_string_repr_ucs2(char *dst/* 6 */,int src/* 0..0xffff */) {
  dst[0]='\\';
  dst[1]='u';
  dst[2]=bb_hexdigit_repr(src>>12);
  dst[3]=bb_hexdigit_repr(src>>8);
  dst[4]=bb_hexdigit_repr(src>>4);
  dst[5]=bb_hexdigit_repr(src);
}

int bb_string_repr(char *dst,int dsta,const char *src,int srcc) {
  if (!dst||(dsta<0)) dsta=0;
  ...
  while (srcp<srcc) {
    switch (src[srcp]) {
      ...
      default: if ((src[srcp]>=0x20)&&(src[srcp]<=0x7e)) {
          ...
          int ch,seqlen;
          if ((seqlen=bb_utf8_decode(&ch,src+srcp,srcc-srcp))<1) {
            ch=(uint8_t)src[srcp];
            seqlen=1;
          }
          if (ch>=0x10000) { // split to surrogate pair
            if (dstc<=dsta-12) {
              ...
              bb_string_repr_ucs2(dst+dstc,hi);
              bb_string_repr_ucs2(dst+dstc+6,lo);
            }
            dstc+=12;
          } else { // bmp
            if (dstc<=dsta-6) {
              bb_string_repr_ucs2(dst+dstc,ch);
            }
            dstc+=6;
          }
          srcp+=seqlen;
...
int bb_string_repr_x(char *dst,int dsta,const char *src,int srcc) {
  if (!dst||(dsta<0)) dsta=0;
  if (!src) srcc=0; else if (srcc<0) { srcc=0; while (src[srcc]) srcc++; }
  int dstc=0,srcp=0;
  if (dstc<dsta) dst[dstc]='"';
    ...
    switch (src[srcp]) {
      ...
      default: if ((src[srcp]>=0x20)&&(src[srcp]<=0x7e)) {
          ...
          if (dstc<=dsta-4) {
            dst[dstc++]='\\';
            dst[dstc++]='x';
            dst[dstc++]=bb_hexdigit_repr(src[srcp]>>4);
            dst[dstc++]=bb_hexdigit_repr(src[srcp]);
          } else {
            dstc+=4;
          }
        }
...
int bb_int_from_json(int *dst,const char *src,int srcc) {
  if (!src) return -1;
  if (srcc<0) { srcc=0; while (src[srcc]) srcc++; }
  if (srcc<1) return -1;
  ...
  if ((src[0]=='-')||((src[0]>='0')&&(src[0]<='9'))) {
    if (bb_int_eval(dst,src,srcc)>=1) return 0; // int, if we got 32 bits properly
    double d;
    if (bb_float_eval(&d,src,srcc)>=0) {
  ...
  if (src[0]=='"') {
    ...
    char tmp[32];
    int tmpc=bb_string_eval(tmp,sizeof(tmp),src,srcc);
    if ((tmpc>0)&&(tmpc<=sizeof(tmp))) {
      if (bb_int_from_json(dst,tmp,tmpc)>=0) return 0;
    }
...
int bb_float_from_json(double *dst,const char *src,int srcc) {
  if (!src) return -1;
  if (srcc<0) { srcc=0; while (src[srcc]) srcc++; }
  if (srcc<1) return -1;
  ...
  if ((src[0]=='-')||((src[0]>='0')&&(src[0]<='9'))) {
    if (bb_float_eval(dst,src,srcc)>=0) return 0;
    int i;
    switch (bb_int_eval(&i,src,srcc)) {
  ...
  if (src[0]=='"') {
    ...
    int tmpc=bb_string_eval(tmp,sizeof(tmp),src,srcc);
    if ((tmpc>0)&&(tmpc<=sizeof(tmp))) {
      if (bb_float_from_json(dst,tmp,tmpc)>=0) return 0;
    }
...
int bb_string_from_json(char *dst,int dsta,const char *src,int srcc) {
  if (!src) return -1;
  if (srcc<0) { srcc=0; while (src[srcc]) srcc++; }
  if (srcc<1) return -1;
  
  // Strings evaluate as you'd expect.
  if (src[0]=='"') return bb_string_eval(dst,dsta,src,srcc);
