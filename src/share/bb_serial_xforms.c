...
static const char *bb_base64_alphabet=
...
int bb_base64_encode(char *dst,int dsta,const void *src,int srcc) {
  ...
  for (;srcc>=3;SRC+=3,srcc-=3) {
    if (dstc<=dsta-4) {
      dst[dstc++]=bb_base64_alphabet[SRC[0]>>2];
      dst[dstc++]=bb_base64_alphabet[((SRC[0]<<4)|(SRC[1]>>4))&0x3f];
      dst[dstc++]=bb_base64_alphabet[((SRC[1]<<2)|(SRC[2]>>6))&0x3f];
      dst[dstc++]=bb_base64_alphabet[SRC[2]&0x3f];
    } else dstc+=4;
  }
  switch (srcc) {
    case 0: break;
    case 1: {
        if (dstc<=dsta-4) {
          dst[dstc++]=bb_base64_alphabet[SRC[0]>>2];
          dst[dstc++]=bb_base64_alphabet[(SRC[0]<<4)&0x3f];
          dst[dstc++]='=';
          dst[dstc++]='=';
        } else dstc+=4;
      } break;
    case 2: {
        if (dstc<=dsta-4) {
          dst[dstc++]=bb_base64_alphabet[SRC[0]>>2];
          dst[dstc++]=bb_base64_alphabet[((SRC[0]<<4)|(SRC[1]>>4))&0x3f];
          dst[dstc++]=bb_base64_alphabet[(SRC[1]<<2)&0x3f];
          dst[dstc++]='=';
        } else dstc+=4;
      } break;
  }
...
int bb_urlencode_encode(char *dst,int dsta,const char *src,int srcc) {
  ...
  for (;srcc-->0;src++) {
    ...
    if (
      ...
      if (dstc<=dsta-3) {
        dst[dstc++]='%';
        dst[dstc++]=bb_hexdigit_repr((*src)>>4);
        dst[dstc++]=bb_hexdigit_repr((*src)&15);
      } else dstc+=3;
    }
  }
  if (dstc<dsta) dst[dstc]=0;
...
int bb_urlencode_decode(char *dst,int dsta,const char *src,int srcc) {
  ...
  while (srcp<srcc) {
    if ((src[srcp]=='%')&&(srcp<=srcc-3)) {
      int hi=bb_digit_eval(src[srcp+1]);
      int lo=bb_digit_eval(src[srcp+2]);
